...
export default function Page() {
  ...
  return (
    <>
      <DatatypeHeader icon={<Database className="w-6 h-6" />} title="Persistent volume claims" count="10" info={info} buttonLabel="Persistent volume claim" />
          ...
          <TableBody>
            {data.map(item => (
              <TableRow key={item.id}>
                <TableCell>
                  <ResourceChip type="primary" badge="PVC" label={item.name} />
                </TableCell>
                <TableCell>
