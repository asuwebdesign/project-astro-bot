...
export default function Page() {
  ...
  return (
    <>
      <DatatypeHeader icon={<Database className="w-6 h-6" />} title="Storage classes" count="10" info={info} buttonLabel="Storage class" />
          ...
          <TableBody>
            {data.map(item => (
              <TableRow key={item.id}>
                <TableCell>
                  <ResourceChip type="primary" badge="SC" label={item.name} />
                </TableCell>
                <TableCell>
